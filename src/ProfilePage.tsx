...
import { api, displayName, displayHandle, hasRealEmail, uploadFile, type Row, type SessionUser } from '../lib/supabase'
...
export function ProfilePage({ user, isAdmin, profile, onSaved }: { user: SessionUser; isAdmin: boolean; profile: Row | null; onSaved: (patch: Row) => void }) {
  ...
  const [contactEmail, setContactEmail] = useState(profile?.contact_email ?? (hasRealEmail(user) ? user.email : '') ?? '')
      ...
      <section className="card">
        ...
        <div className="px-5 sm:px-6 py-6 flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[13px] text-muted-foreground truncate">{displayHandle(user)}</p>
